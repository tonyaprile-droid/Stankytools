...
def _write_windows_update_script(zip_path: Path, app_dir: Path, exe_path: Path) -> Path:
    ...
    ps = (
        ...
        # Copy application files but never overwrite user data/log/update cache.
        f"Get-ChildItem -LiteralPath '{extract_dir}' -Force | Where-Object {{ $_.Name -notin @('data','logs','updates') }} | "
        f"ForEach-Object {{ Copy-Item -LiteralPath $_.FullName -Destination '{app_dir}' -Recurse -Force }}; "
