...
NAV_ICON_BADGE_SIZE = 42
USER_PANEL_HEIGHT = 58
SETTINGS_PANEL_RADIUS = 16
SETTINGS_CARD_HEIGHT = 70
SETTINGS_CARD_SPACING = 9
...
class ThemedSidebar(QFrame, CoverImageMixin):
    ...
    def paintEvent(self, event):
        ...
        veil.setColorAt(0.0, QColor(0, 0, 0, 150))
        veil.setColorAt(0.34, QColor(0, 0, 0, 112))
        veil.setColorAt(0.74, QColor(0, 0, 0, 120))
        veil.setColorAt(1.0, QColor(0, 0, 0, 162))
        painter.fillRect(r, veil)
        panel = QRectF(12, 226, self.width() - 24, 410)
        painter.fillRect(panel, QColor(0, 0, 0, 82))
...
class AngularNavButton(QAbstractButton):
    ...
    def resizeEvent(self, event):
        self.icon.move(10, max(0, (self.height() - NAV_ICON_BADGE_SIZE) // 2))
    ...
    def _shape(self) -> QPainterPath:
        r = QRectF(30, 6, self.width() - 40, self.height() - 12)
        path = QPainterPath()
    ...
    def paintEvent(self, event):
        ...
        if self.active or self.hover:
            p.setPen(QPen(with_alpha(accent, 62 if self.active else 42), 7))
            p.drawPath(shape)

        fill = QLinearGradient(QPointF(30, 0), QPointF(self.width() - 10, 0))
        fill.setColorAt(0.0, QColor(0, 0, 0, 184 if self.active else 158))
        fill.setColorAt(0.55, QColor(0, 0, 0, 154 if self.active else 132))
        fill.setColorAt(1.0, QColor(0, 0, 0, 118 if self.active else 100))
        if self.hover:
        ...
        font = QFont("Rajdhani", 15, QFont.Bold)
        font.setLetterSpacing(QFont.PercentageSpacing, 104)
        p.setFont(font)
        p.setPen(QColor("#FFFFFF"))
        text_rect = QRectF(76 + dx, yoff, self.width() - 86, self.height())
...
class ThemeSelectionCard(QAbstractButton):
    ...
    def __init__(self, key: str, label: str, active_key: str = "dune", parent=None):
        ...
        self.setText(label); self.setCursor(Qt.PointingHandCursor); self.setFixedHeight(50); self.setSizePolicy(QSizePolicy.Expanding,QSizePolicy.Fixed)
    ...
    def paintEvent(self,event):
        ...
        emblem = QRectF(r.left() + 12, r.center().y() - 16, 32, 32)
        p.setBrush(QColor(0, 0, 0, 70))
        p.setPen(QPen(with_alpha(accent, 170), 1.0))
        p.drawEllipse(emblem)
        p.setPen(QPen(with_alpha(accent, 190), 1.1))
        p.drawEllipse(emblem.adjusted(5, 5, -5, -5))
        for i in range(8):
            a = i * 45
            import math
            x1 = emblem.center().x() + math.cos(math.radians(a)) * 5
            y1 = emblem.center().y() + math.sin(math.radians(a)) * 5
            x2 = emblem.center().x() + math.cos(math.radians(a)) * 12
            y2 = emblem.center().y() + math.sin(math.radians(a)) * 12
            p.drawLine(QPointF(x1, y1), QPointF(x2, y2))

        p.setFont(QFont("Rajdhani", 13, QFont.Bold))
        p.setPen(accent)
        p.drawText(QRectF(r.left() + 56, r.top(), r.width() - 94, r.height()), Qt.AlignVCenter | Qt.AlignLeft, self.text())
...
class SettingsActionCard(QAbstractButton):
    def __init__(self, title: str, description: str, icon_type: str, theme_key: str = "dune", parent=None):
        ...
        self.setText(title.upper()); self.setCursor(Qt.PointingHandCursor); self.setFixedHeight(SETTINGS_CARD_HEIGHT); self.setSizePolicy(QSizePolicy.Expanding,QSizePolicy.Fixed)
    ...
    def paintEvent(self,event):
        ...
        icon_r = QRectF(r.left() + 14, r.center().y() - 22, 44, 44)
        p.setBrush(QColor(0, 0, 0, 112))
        p.setPen(QPen(with_alpha(accent, 88 if not self.hover else 145), 1.0))
        p.drawEllipse(icon_r)
        self._draw_action_icon(p, icon_r, accent)

        dx = 4 if self.hover else 0
        yoff = 1 if self.pressed_inside else 0
        p.setPen(accent)
        title_font = QFont("Rajdhani", 14, QFont.Bold)
        title_font.setLetterSpacing(QFont.PercentageSpacing, 108)
        p.setFont(title_font)
        p.drawText(QRectF(r.left() + 72 + dx, r.top() + 10 + yoff, r.width() - 126, 23), Qt.AlignLeft | Qt.AlignVCenter, "WORKING..." if self.busy else self.text())
        p.setPen(QColor("#B9B0A4"))
        p.setFont(QFont("Segoe UI", 9))
        p.drawText(QRectF(r.left() + 72 + dx, r.top() + 35 + yoff, r.width() - 126, 20), Qt.AlignLeft | Qt.AlignVCenter, self.description)
...
class UpdateStatusCard(QAbstractButton):
    def __init__(self, theme_key: str = "dune", parent=None):
        super().__init__(parent); self.theme_key=visual_key(theme_key); self.status_text="You are up to date"; self.hover=False; self.setCursor(Qt.PointingHandCursor); self.setFixedHeight(38); self.setSizePolicy(QSizePolicy.Expanding,QSizePolicy.Fixed)
    def set_theme(self,key): self.theme_key=visual_key(key); self.update()
